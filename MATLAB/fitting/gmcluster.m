...
elseif ischar(start) 
    ...
    elseif strncmpi(start,'plus',length(start))
        start='plus';
    % Mandy create a case for customize
    elseif strncmpi(start, 'customize', length(start))
        start='customize';
    else
...
for t = 1:reps
    switch start
        ...
        % Mandy adds to specify initial mean
        case 'customize'
            initPara = customizeParam(X,k, CovType, SharedCov,RegV);
    end
...
%         fprintf('current log likelihood diff is %4.2f and target is %4.2f\n',llDiff,options.TolFun *abs(ll));
%     end
...
%------------------
% Mandy: get initial parameters using customized mean values
function initPara = customizeParam(X,k, CovType, SharedCov,RegV)
initPara.PComponents = ones(1,k,'like',X)/k ;% equal mixing proportions
...
for i = 1:k
    C(i,:) = zeros(1,length(X(1,:))) + 0.1*rand(1,length(X(1,:)));
end
if length(X(1,:))==3
