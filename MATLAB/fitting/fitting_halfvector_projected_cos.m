...
newweight = zeros(generatenum,1);
for i = testafter+1:length(x)
    if z(i)>=0
        h = ([x(i),y(i),z(i)] + incident)/2;
        h = h/norm(h);  
        result(ceil((h(1)+1)/x_unit),ceil((h(2)+1)/y_unit)) = ...
            result(ceil((h(1)+1)/x_unit),ceil((h(2)+1)/y_unit)) + weight(i)*h(3)^4;
        newweight(i-testafter) = weight(i)*h(3)^4;
    end
end

% close all
figure
imagesc(result/sum(newweight))
colorbar()
...
newindex = randsample((1:trainnum)',trainnum,true,...
    weight(1:trainnum).*factor/sum(weight(1:trainnum).*factor));
...
%     saveas(gcf,[filename,'.jpeg'])
    ...
    %% calculate error
    diff = predict/(generatenum-count)-result/sum(newweight);
